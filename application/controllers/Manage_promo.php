...
class Manage_promo extends CI_Controller
{
    var $module_js = ['manage-promo'];
    var $app_data = [];
    ...
    public function check_unique_title($judul)
    {
        ...
        $query = $this->db->get('produk_promo');
    ...
    public function edit_data()
    {
        ...
        $data_produk = $this->data->find('produk_promo', array('id' => $id, 'is_deleted' => '0'))->row_array();
        ...
        if ($this->form_validation->run() == false) {
            ...
            if (empty($this->input->post('kategori'))) {
                ...
                $where = array('id' => $id);
                $updated = $this->data->update('produk_promo', $where, $data);
    ...
    public function delete_data()
    {
        ...
        if ($updated) {
            $response = "<script>$(document).ready(function () {
                ...
                Toast.fire({
                    icon: 'success',
                    title: 'Anda telah berhasil hapus data'
                  })
              });</script>";
        }
        echo json_encode($response);
